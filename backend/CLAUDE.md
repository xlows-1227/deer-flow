...
`POST /api/v1/auth/login` is the unified first-call login endpoint and `POST /api/v1/auth/login/local` is the explicit local-provider endpoint. Both exact paths and their trailing-slash forms are public and CSRF-exempt; do not treat the unified route as a removed legacy path.
...
| **Files** (`/api/files`) | `GET /` - list user library; `GET /folders` - folder list; `POST /upload` - upload (rejects all reserved system roots); `GET /{path}` - download/preview; `DELETE /{path}` - delete |
| **File Shares** (`/api/file-shares`) | `POST /` - share an owned library/conversation file with a registered account email; `GET /` - list live files shared with the current user; `GET /{id}/content` - recipient-only read/download (active HTML/SVG content is served as plain text unless downloaded) |
...
The web file manager presents thread attachments and `ThreadState.artifacts` through synthetic root collections, `对话上传` and `对话生成`, and received file-share references through `他人分享`. Source files remain in their owner/thread storage rather than being copied. All three roots are locked in the UI, omitted from manual upload destinations, and reserved by the Files API. Because thread search returns only the display title in `values`, the generated collection loads each bounded search result's latest checkpoint state to obtain `artifacts`. File-share creation validates the recipient against the registered `users` table and re-checks thread ownership for conversation sources; recipients receive read/download access only. Markdown uses the normal renderer, while HTML is fetched as non-executable text and rendered by the frontend with `allow-scripts allow-forms` in an opaque-origin sandbox (no `allow-same-origin`).
...
- `draft_service.py` — `DraftService`: create/edit agents, partial draft updates (revision-conflict → 409), skill selection (ownership-checked), connector grants checked against both owner/active state and authoritative Connector-type capabilities, lifecycle (suspend/resume/archive, never delete). Studio option rows retain the Skill catalog's `display_name`, English `description`, and `description_zh`; these presentation fields do not participate in authorization.
- `validation.py` — `validate_draft_for_publish()`: pure function implementing the publish rules (design §8.2); aggregates all violations rather than fail-fast, and only counts grants whose current Connector type supports the granted capability.
- `publish_service.py` — `PublishService.publish()` captures draft + skill/grant children in one SQL snapshot. Skill file trees are captured twice and bracketed by fresh enabled/visibility/owner reads; connector requirements are parsed from the exact frozen `SKILL.md` bytes, and same-checksum revision metadata mismatches fail closed. Tool groups are platform-managed for database Studio Agents: draft/sandbox runtime ignores legacy draft selections, and each new immutable Release snapshots the current platform group whitelist. Historical Releases and legacy filesystem Agents remain unchanged. The release transaction locks identity then draft, rechecks the revision, pins Skill revisions, inserts the immutable release, and switches the pointer. Concurrent authoring returns `DRAFT_REVISION_CONFLICT` (HTTP 409) without a release. `rollback()` repoints `current_release_id` only. Studio drafts default to `explicit + []`; only conversational/legacy inputs that omit skills use `inherit`.
...
- `instructions.py` — `compose_agent_instructions()`: AGENT.md then SOUL.md into labelled prompt blocks. This remains the runtime compatibility contract for legacy drafts and immutable Releases; Studio keeps AGENT.md user-editable from a complete work-rules template while SOUL.md is generated from a managed personality preset. Legacy custom SOUL content remains read-only until the owner explicitly selects a preset.
- `import_service.py` — `AgentImportService`: imports legacy `users/{uid}/agents/{name}/` filesystem agents as `status=draft` rows; identity, draft, and Skills commit in one transaction so failures leave no partial slug and retries are safe (never auto-publishes, never deletes source files).
...
| GET    | `/{agent_id}/draft/options`                | Owner-authorized Studio Skill catalog with localized metadata and declared Connector capability requirements         |
| POST   | `/{agent_id}/draft/sandbox-runs`           | Start a non-billable Run from the saved draft and freeze its revision/capability scope                               |
| GET    | `/draft/sandbox-threads/{thread_id}`       | Return the owner-only frozen Skill and Connector scope used by the sandbox chat composer                             |
| GET    | `/{agent_id}/usage?days=&source=&key_id=`  | Owner-scoped daily usage; source/credential filters are part of the SQL predicate                                    |
| GET    | `/{agent_id}/quota`                        | Platform defaults + draft owner overrides + bounded next-Release effective values                                    |
| GET    | `/{agent_id}/audit?limit=`                 | Recent rejection metadata only; response omits content and identifying hashes                                        |
| PATCH  | `/{agent_id}/draft`                        | Update draft (revision required; 409 on conflict; 422 on malformed/duplicate/unselectable Skill or Connector grants) |
...
The conversational `setup_agent` / `update_agent` tools are **async LangChain tools** (`async def`) that directly `await` `DraftService` on the Gateway's event loop — no executor and no cross-loop `AsyncEngine` sharing. Setup (identity + draft + skills) and update (metadata + all draft fields + skills) each use one SQLAlchemy session and one commit. All paths that lock both authoring rows use the global `published_agents` identity → `agent_drafts` order, including publish. Persistence mode writes only the database: before the synchronous graph factory runs, the async Gateway worker removes caller-supplied `__agent_*` fields and hydrates the owner-scoped draft into the run config, including combined AGENT→SOUL instructions, skill-selection mode, and draft revision. A confirmed database miss may read only that owner's legacy `SOUL.md` / `config.yaml` during migration; database/query errors, another owner's files, and the shared legacy directory never trigger fallback. When persistence is unavailable (CLI), files are the sole source of truth and a best-effort multi-file transaction compensates normal write errors. Sync embedded custom-agent runs are rejected while a live async database engine exists; default-agent embedded flows remain supported. Unresolved skills are listed in the ToolMessage.

Draft-sandbox metadata keys are server-reserved on Thread create/PATCH and are stripped from ordinary Run metadata before persistence. Initial sandbox Runs also persist `agent_name` plus `agent_display_name` for Agent-aware chat-history routing and labels. `start_run()` detects a trusted sandbox Thread and reconstructs `DraftSandboxContext` before every follow-up message, restoring the exact Skill list and Connector capability map. Reconstruction is permitted only while the monotonic draft revision still matches; a changed draft returns `draft_sandbox_revision_stale` (409) and requires a new sandbox conversation.
...
**Final Published-Agent acceptance gate (M4):**

- `tests/test_acceptance_multi_tenant.py` maps design §19 items 1–5 and 8–14
  one-to-one onto real SQLite repositories and publishing services.
- The matching frontend gate is
  `frontend/tests/e2e/multi-tenant-acceptance.spec.ts`; it covers instruction
  combinations, draft/live separation, publish-before-Key, and late Feishu
  binding.
- Public curl examples must use the Agent API request contract
  `{"message":"..."}`. Do not substitute the internal LangGraph
  `input.messages` shape.
- Operational procedures and invariants are maintained in
  `../docs/PUBLISHED_AGENTS.md`.
...
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| `persistence/agent_api_key/`         | Slow-salted hashes, one-time plaintext creation, multiple active named Keys, delete/rotate/revoke/expiry and Key overrides |
...
Studio operations filtering stays below the router boundary:
`AgentUsageRepository.aggregate_daily()` accepts optional source and credential
predicates, while `ExternalAuditRepository.list()` applies the minimum status
code before ordering and limiting. The audit route then allowlists operational
metadata only; request/response bodies, actor hashes, client IP hashes, and
User-Agent are never returned. Quota policy reads the runtime
`PublishingConfig.platform_quota`; draft overrides remain revisioned authoring
state and do not affect the current Release until the next publish.
...
**Provider Pattern**: `SandboxProvider` with `acquire`, `acquire_async`, `acquire_with_lease_async`, `accept_acquisition`, `abandon_acquisition`, `get`, and `release` lifecycle. Async agent/tool paths call async sandbox lifecycle hooks so Docker sandbox creation, discovery, cross-process locking, readiness polling, and release stay off the event loop. Timeout-sensitive callers use `SandboxAcquisition` handles; AIO conditionally abandons only capacity introduced by that acquisition and protects same-thread reuse with accepted-use generations.
**Implementations**:
...
**Published Feishu flow**: app-credential-authenticated SDK WebSocket (`im.message.receive_v1` and `card.action.trigger`) → application event-header token comparison + timestamp replay check → durable `(binding_id, event_id)` claim → MessageBus → stable binding/Agent DB mapping → Published resolver → cheap declared-input checks → quota reserve → authenticated streaming attachment materialization → Gateway Run with Published runtime policy → terminal usage settlement → outbound card. Card button/confirm callbacks from Skill-sent cards use the same long connection and become a new inbound chat payload (`{"type":"feishu_card_action",...}`); they do not resume the previous Run. The SDK long-connection path exposes no HTTP callback signature and internally uses `do_without_validation`, so the application token check is mandatory. Verification and dedup occur before quota. Same event ids on different bindings are independent. Group card clicks should put `chat_type` / `topic_id` in the button `value` so mapping matches the original conversation.
...
- M3 v1 runs exactly one Gateway process with the Published Feishu Supervisor. Set one stable `DEER_FLOW_SECRET_STORE_KEY` on that process. Generate it with `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"`. Ciphertext lives at `${DEER_FLOW_HOME:-.deer-flow}/secret-store/feishu/`; the database stores opaque refs only. Never log or return the decrypted `app_secret`, `verification_token`, or `encrypt_key`.
...
- The Supervisor acquires `${DEER_FLOW_HOME:-.deer-flow}/published-feishu-supervisor.lock` before startup recovery; stop all pre-fence Gateways before rolling out this build. Runtime claims are persisted and re-read before WebSocket open, the provisional lease is renewed until readiness, and confirmation happens only after ready. Explicit operations and startup reload share one convergence helper and select from its closed named policy enum: the normal path from initial row re-read through final health projection has a 25-second per-binding deadline, cancellation teardown has one shared 2-second budget for tracked start/renewal/stop/release tasks, and fallback health projection has a separate 2-second bound. Runtime claims are deadline-owned tasks, never operations under a cross-binding lock. Timeout, exception, and self-cancellation are all outcome-ambiguous even when the client task is already done. Every such claim remains explicit process ownership until a system-scoped row-lock transaction serializes with any in-flight commit and clears only its exact token; transient reconciliation errors retain the same retry owner. Exact-token reconciliation projects startup failure into the newly released health epoch only while the expected claim generation still identifies that failed attempt, returns a named durable result, and never overwrites a foreign successor. Process-local failure health is published only after a durable fingerprint re-read still matches that result. Shutdown drains both late claim/release ownership and every quiescing transport/release owner inside one 20-second Supervisor budget, and the production Gateway wrapper allows that full budget plus the normal 5-second scheduling margin while keeping ordinary hooks capped at 5 seconds. Unresolved ownership raises without marking shutdown complete or releasing the leader fence; a later call on the same Supervisor retries convergence. A transport task that does not acknowledge cancellation remains attached to a quiescing cleanup retry owner and keeps its fencing token; it cannot be replaced before confirmed exit. Timeout, database failure, renewal failure, or token mismatch records only that binding unhealthy; ready peers remain managed. Lease timestamps never authorize takeover, STOP, or physical delete. A matching explicit release after `stop()` returns is the normal acknowledgement; only a newly acquired OS leader fence proves a hard-killed old process is gone and authorizes clearing its orphaned tokens. Cancelled fence acquisition drains the blocking worker and releases a late-acquired OS lock before propagating cancellation. Readiness is not exit confirmation: failed stop/release retains one quiescing retry owner even after `is_running=False` and rejects replacement. POST/PATCH move `agent_channel_secret_ingests` through `reserved → writing(token,generation,lease) → ready`, heartbeat slow writes, and atomically transfer only ready rows during create/stage. Janitors claim expired writers but retain the DB cleanup owner when ciphertext is still absent, so a paused atomic writer cannot create an orphan. Legacy `.pending` cleanup is paused during rolling upgrades unless the binding already owns the ref. Tombstones/stop fences reject start/restart/rotation, and retryable cleanup returns `409`.
- `_record_health()` always sends the expected durable `runtime_generation`, `runtime_lease_token`, and a monotonic `health_revision`. `AgentChannelRepository.update_health()` accepts only a newer matching observation, and local health changes only when that durable CAS succeeded and the observation is still process-locally latest. `update_credentials()` advances `runtime_generation` before resetting health/revision, making credential replacement a new health epoch for both inactive and active bindings; an old-secret probe therefore becomes stale before the active runtime restart. Detached stale writers are result-consumed but cannot block peers or overwrite a successor or a newer same-generation observation. Persistence failures/timeouts return a call-local fallback without publishing it into the shared health snapshot. Use `running_binding_ids` for serving/non-quiescing transports and `owned_binding_ids` only for internal fencing/cleanup convergence; runtime health callbacks must use the same serving predicate. A credential probe invalidated by restart or credential rotation returns the current durable health rather than retrying its stale write or escaping as HTTP 500.
...
- Attachment admission happens after Published resolver and quota reservation. It permits at most 10 resources, 50 MiB each, and an effective aggregate `UTF-8 text + actual bytes` limit. Published resources use authenticated streaming HTTP with bounded connect/read/overall deadlines. AIO `creating` and active lifecycle records carry a provider instance ID, generation, and renewable lease under the sandbox file lock. Periodic discovery is observational, and explicit discovery refuses another provider's live lifecycle lease; only the same owner or an expired record may adopt. Late compensation and destroy revalidate generation/owner.
- Attachment cleanup remains a fenced JSON outbox with bounded global recovery. Before request admission, two disposable scan workers and their process-owned pool manager are prestarted. Owner `DELETE` and binding startup cleanup-health projection send the complete directory-enumeration/stat/size/read/parse operation to one worker with a 2-second budget; neither path starts a process or performs a filesystem scan. Timeout terminates/kills that worker while the prestarted standby remains usable; invalid state, concurrency and worker exhaustion fail closed with `409`. The background manager replenishes lost slots with bounded backoff and rolls back partial prestart failures. Readiness polling checks shutdown in short slices. Every terminate/kill path performs a final `is_alive()` check; successful shutdown returns only after the manager and all published or unpublished children have exited. A stubborn child remains tracked, keeps the scanner stop fence set, raises shutdown failure, and blocks a new lifespan until a later lifecycle check confirms its exit.
- Scanner process API failures are exit-unknown, not exit-confirmed. `is_alive/terminate/join/kill` exceptions are logged while later best-effort steps continue; request I/O and response-deserialization exceptions use the same failure helper; scan, replenish rollback and shutdown retain every unconfirmed slot, set the stop fence and reject restart. Discovery cursor replacement retries transient Windows sharing violations within both the injected logical deadline and a monotonic wall-clock bound. The helper checks both budgets before every `Path.replace()`: expiry before the first attempt raises `TimeoutError`, while expiry after contention re-raises the last `PermissionError`, preventing a timed-out worker from beginning a late cursor publication. Cursor tests cover first-attempt expiry, retry-sleep exhaustion, frozen logical clocks and transient success. Remaining real-deadline and Supervisor contention tests use explicit barriers, retries and expanded observation budgets. Runtime release- and stop-failure tests share one entered/release/recovered barrier, durable-token wait, quiescing-owner wait, conditional shutdown and error-grouping helper. Every `finally` releases the test barrier, but unresolved ownership is a cleanup failure: no retry shutdown runs and no leader fence is manually released. Gateway engine disposal executes independently, and `BaseExceptionGroup` preserves body, ownership-cleanup and disposal failures together. Router tests use the same pytest-asyncio runner as repository/supervisor tests, release every barrier in `finally`, join request tasks, and stop the process-owned scanner at module boundaries. Each Router engine and Supervisor is registered with one async resource fixture at creation time; fixture teardown always attempts both Supervisor shutdown and database disposal, preserves a single original failure, and raises one grouped failure when both cleanup operations fail so neither cause is lost.
- The trusted `owner_user_id` is explicit through the complete Published `start_run()` lifecycle: owner effective config and model validation, Run/Thread SQL persistence, worker creation, host paths, and sandbox acquisition all execute inside the owner scope. Local/AIO providers bind cached thread entries to that owner and fail closed on conflict; deterministic AIO sandbox IDs include owner scope. Child Run workers retain the real owner ContextVars for their lifetime while the caller context is restored on success, failure, and cancellation, so middleware, uploads, outputs, and final attachment resolution stay in the same tenant.
- A Run-bound quota reservation cannot use ordinary release. Every exception or cancellation after `run_starter` succeeds has explicit started/detached semantics. Dispatcher cancellation cancels and joins the started Run before terminal settlement; cancellation RPC and worker join have independent short cleanup deadlines, and any unconfirmed cleanup leaves the reservation pending for durable recovery. `release_unstarted` is limited to failures before Run creation succeeds. `PatchedChatDeepSeek` conservatively counts the canonical message payload for compatible aliases that LangChain's `gpt-*`-only message counter rejects. `PublishedRunTokenLimitError` bypasses `LLMErrorHandlingMiddleware` so preflight/policy failures reach the Run worker and persist as `error`, never as a successful empty Published response.
- Gateway startup auto-applies Alembic through head `2026_07_30_widen_cred_ref`; the chain includes `2026_07_13_file_shares → 2026_07_12_published_agents → 2026_07_14_agent_channels → 2026_07_14_channel_mappings → 2026_07_17_channel_deletion_state → 2026_07_26_agent_ops_metrics` and the monotonic `agent_channels.health_revision` fence. Keep the existing file-sharing migration immediately before the Published Agent migrations so databases stamped at that historical revision remain upgradeable. When diagnosing, inspect the redacted `health`/`health_detail`, call the `test` route, verify the deployment key is mounted consistently, and look for ready/stop/runtime-loss log events.
...
pytest tests/test_agent_channel_repo.py tests/test_agent_channels_router.py \
  ...
  tests/test_user_context.py tests/test_gateway_services.py tests/test_secret_store.py \
...
- [PUBLISHED_AGENTS.md](../docs/PUBLISHED_AGENTS.md) - Published-Agent deployment and operations
