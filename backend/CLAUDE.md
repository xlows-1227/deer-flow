...
The web file manager presents thread attachments and `ThreadState.artifacts` through synthetic root collections, `对话上传` and `对话生成`, and received file-share references through `他人分享`. Source files remain in their owner/thread storage rather than being copied. All three roots are locked in the UI, omitted from manual upload destinations, and reserved by the Files API. File-share creation validates the recipient against the registered `users` table and re-checks thread ownership for conversation sources; recipients receive read/download access only. Markdown uses the normal renderer, while HTML is fetched as non-executable text and rendered by the frontend in a script-disabled sandbox.
