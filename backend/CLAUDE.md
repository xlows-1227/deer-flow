...
- Gateway startup auto-applies Alembic `2026_07_14_agent_channels → 2026_07_14_channel_mappings → 2026_07_17_channel_deletion_state`; the head includes the monotonic `agent_channels.health_revision` fence. When diagnosing, inspect the redacted `health`/`health_detail`, call the `test` route, verify the deployment key is mounted consistently, and look for ready/stop/runtime-loss log events.
