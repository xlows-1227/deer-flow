...
- Desired state and redacted health live in `agent_channels`. WebSocket ready is independent from sandbox recovery: startup performs only a fast local backlog projection, then a per-binding coordinator runs recovery in the background. Supervisor lifecycle locks are ref-counted per binding and cover start/stop/restart/credential rotation plus row deletion; physical delete completes before unlock, startup re-reads list snapshots, and retired keyed locks disappear only after all waiters exit. A pending cleanup outbox keeps the running binding unhealthy; transient health repository failures leave the projection dirty and are retried by the next pass. Late connection failure marks only that binding unhealthy. Missing/invalid deployment key logs `Published Feishu Supervisor unavailable` and disables only DB bindings, not publication or legacy channels.
...
- Attachment admission happens after Published resolver and quota reservation. It permits at most 10 resources, 50 MiB each, and an effective aggregate `UTF-8 text + actual bytes` limit. Published resources bypass the lark-oapi binary helper (which buffers `resp.content`) and use authenticated `httpx` streaming with 5 s connect, 10 s read, and 60 s overall deadlines, `Content-Length` preflight, and per-raw-chunk accounting. Sandbox acquisition uses `acquire_with_lease_async()` with a 15 s foreground deadline and a bounded late-compensation deadline. Success calls provider `accept_acquisition`; timeout/cancellation calls provider `abandon_acquisition` only after a typed handle exists. AIO backend creates carry operation tokens that remain observable until the blocking call really completes, even past the 120 s warning deadline. Late compensation re-enters the thread/file-lock protocol and checks active mapping plus accepted-use generation before destroy; OS file-lock cancellation hands eventual unlock/close to the worker. Non-mounted sync uses 60 s per-file and 120 s batch deadlines.
- Rejection, sync timeout, and cancellation remove host partials and non-mounted sandbox copies. Critical cleanup tasks are separate from disposable card/progress tasks: stop drains them for 2 s without cancelling them. The per-job JSON outbox under `${DEER_FLOW_HOME:-.deer-flow}/published-attachment-cleanup/` is a persisted `producer_pending → ready_to_delete → deleting` state machine. Producer leases heartbeat until the uncancellable sync worker exits; delete claims use file-locked fencing and expire after crashes. The global coordinator parses each JSON candidate once inside one 10 s total deadline, chooses at most 25 claimable jobs across all bindings, limits execution to four jobs, and advances a persisted rotating cursor; active producer/claim leases are excluded before selection. Each delete call has a 2 s deadline and three bounded remote retries. Per-binding recovery starts after ready, while a Gateway-level 30 s janitor performs the same row/secret-independent global pass. Cleanup health uses local generation CAS plus a file-locked durable store generation and final durable projection, so stale snapshots cannot clear newer backlog. `DELETE` returns 409 and retains the row/secret while backlog exists. The outbox completes only after remote and host deletion are confirmed.
...
pytest tests/test_agent_channel_repo.py tests/test_agent_channels_router.py \
  ...
  tests/test_user_context.py tests/test_gateway_services.py -q
```
