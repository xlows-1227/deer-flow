...
| **HTML 对外发布** (`/api/file-publications`, `/api/public-files`) | 登录用户通过 `POST /api/file-publications` 发布对话生成的 HTML，通过 `GET /api/file-publications` 查询自己的发布记录，通过 `DELETE /api/file-publications/{id}` 取消发布；外部访问者无需登录即可读取 `GET /api/public-files/{token}` 元数据和 `GET /api/public-files/{token}/content` HTML 文本。 |
...
网页文件管理器通过“对话上传”和“对话生成”两个虚拟根目录展示对话附件与 `ThreadState.artifacts`，并通过“他人分享”展示接收到的分享引用。源文件始终保留在所有者或对话存储中，不做额外复制；三个系统目录均在界面中锁定、从人工上传目标中排除，并被 Files API 保留。由于对话搜索结果的 `values` 只包含显示标题，“对话生成”需要为限定数量的搜索结果读取最新 checkpoint 才能获得 `artifacts`。创建定向分享时必须在 `users` 表中验证接收邮箱，并重新校验对话来源的所有权；接收者只有读取和下载权限。Markdown 使用普通渲染器，HTML 以不可执行文本读取，再由前端放入允许脚本和表单、但不含 `allow-same-origin` 的不透明源沙箱。

HTML 对外发布使用独立的 `file_publications` 表，与面向注册接收人的定向分享解耦。发布接口只接受当前用户拥有的对话中、位于 `/mnt/user-data/outputs/` 下的 `.html` 或 `.htm` 文件；`(owner_user_id, thread_id, source_path)` 唯一约束保证重复发布幂等，`public_token` 提供稳定永久地址。公开内容接口固定以 `text/plain; charset=utf-8` 返回源码，并设置 `X-Content-Type-Options: nosniff` 和 `Cache-Control: no-store`，前端再通过 `srcDoc` 注入不含 `allow-same-origin` 的 iframe。读取时必须重新校验源文件存在且设备号/inode 标识未变化；删除、替换或所有者取消发布后统一返回 404，不暴露具体失效原因。明确重新发布替换文件时更新对象标识但保留原 token。
