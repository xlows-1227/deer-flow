...
- `draft_service.py` — `DraftService`: create/edit agents, partial draft updates (revision-conflict → 409), skill selection (ownership-checked), connector grants checked against both owner/active state and authoritative Connector-type capabilities, lifecycle (suspend/resume/archive, never delete).
