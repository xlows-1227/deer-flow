...
async def test_create_returns_plaintext_once_and_persists_only_slow_hash(key_repo) -> None:
    ...
    created = await repo.create(agent_id="pa_1", name="Production", quota_overrides={"daily_runs": 10})
    ...
    listed = await repo.list_by_agent("pa_1")
    assert len(listed) == 1
...
async def test_verify_revocation_and_multiple_active_keys(key_repo) -> None:
    ...
    keys = [await repo.create(agent_id="pa_1", name=f"Key {index}") for index in range(3)]

    verified = [await repo.verify(item["api_key"]) for item in keys]
    assert all(item is not None for item in verified)
    assert len(await repo.list_by_agent("pa_1")) == 3

    assert await repo.revoke("pa_1", keys[1]["id"]) is True
...
async def test_rotation_keeps_old_key_during_overlap_then_expires_it(key_repo) -> None:
    ...
    old = await repo.create(agent_id="pa_1", name="Partner")

    rotated = await repo.rotate("pa_1", old["id"], overlap_seconds=3600)
    ...
    assert await repo.verify(rotated["api_key"]) is not None
    statuses = {item["id"]: item["status"] for item in await repo.list_by_agent("pa_1")}
...
async def test_verify_rejects_malformed_unknown_and_tampered_keys(key_repo) -> None:
    repo, _session_factory, _clock = key_repo
    created = await repo.create(agent_id="pa_1", name="Key")
...
async def test_update_is_scoped_to_agent(key_repo) -> None:
    repo, _session_factory, _clock = key_repo
    created = await repo.create(agent_id="pa_1", name="Old")

    assert await repo.update("pa_other", created["id"], name="Stolen") is None
    updated = await repo.update("pa_1", created["id"], name="New", quota_overrides={"daily_runs": 5})
