...
function isOptimisticMessage(message: Message): boolean {
  // Check if a message is an optimistic (client-side) message.
  const id = message.id || "";
...
function sortMessagesByTime(messages: Message[]): Message[] {
  ...
  // Separate optimistic messages from server messages
  ...
  // Sort server messages by time/seq
  ...
  // If there are no optimistic messages, return sorted server messages
  if (optimisticMsgs.length === 0) {
    return sortedServerMsgs.map(({ m }) => m);
  }
  
  // For optimistic messages, we need to preserve their relative position
  // in the original message list. Optimistic messages should maintain
  // their order relative to each other and should not be sorted past
  // server messages that came before them.
  
  // Strategy: Place optimistic messages after the last server message
  // that appears before them in the original order
  const result: Message[] = [];
  let optIdx = 0;
  
  // Merge sorted server messages with optimistic messages
  // Optimistic messages should be placed after the last server message
  // that had a lower original index
  for (const serverMsg of sortedServerMsgs) {
    const origIdx = messages.indexOf(serverMsg.m);
    
    // Add any optimistic messages that originally appeared before this server message
  ...
  // Add remaining optimistic messages at the end
  while (optIdx < optimisticMsgs.length) {
    result.push(optimisticMsgs[optIdx]!);
    optIdx++;
  }
...
function humanMessageFilesSignature(message: Message): string | null {
  ...
  // 1. structured files (from optimistic UI + server re-serialization)
  ...
  // 2. textual <uploaded_files> marker inside content (server echo)
...
function hasServerReplacementForOptimisticHuman(
  ...
  // If we have neither text-based nor file-based identities (shouldn't
  // really happen — a human message with no files and no text wouldn't be
  // submitted), bail to the previous conservative behaviour.
...
export function getVisibleOptimisticMessagesForServerMessages(
  ...
  // Debug: log optimistic messages and server messages for troubleshooting
  if (typeof window !== 'undefined') {
    const debugInfo = {
      hasHumanOptimistic,
      optimisticHumanMessages: optimisticMessages
      ...
      serverHumanMessages: serverMessages
        .filter(m => m.type === 'human')
        .map(m => ({
          id: m.id,
          content: typeof m.content === 'string' ? m.content.substring(0, 50) : '[non-string]',
          additionalKwargs: m.additional_kwargs,
          hideFromUI: m.additional_kwargs?.hide_from_ui,
          viewImageContext: m.additional_kwargs?.view_image_context,
        })),
      baselineKeys: Array.from(baselineHumanMessageKeys),
    };
    console.log('[DEBUG] getVisibleOptimisticMessagesForServerMessages:', debugInfo);
  }
  ...
  if (hasHumanOptimistic) {
    ...
    console.log('[DEBUG] hasServerReplacementForOptimisticHuman:', hasReplacement);
    if (hasReplacement) {
      return [];
    }
  }
  ...
  const thread = useStream<AgentThreadState>({
    ...
    onCustomEvent(event: unknown) {
      ...
      if (
        ...
        // Don't show intermediate retry toasts at all — they flicker too
        // fast and confuse users (e.g. "上来就是 3/3").  We only surface
        // the FINAL failure via the message bubble.  Retries are invisible
        // to the user; only the final result matters.
        if (e.attempt >= e.max_attempts) {
          // Last attempt — show a single toast acknowledging the retry
          // limit was hit.  This is the only time the user sees retry UI.
          toast("LLM 服务繁忙，已达最大重试次数", { id: "llm-retry-final", duration: 5000 });
        }
        // All intermediate retries are silently swallowed.
      }
    },
    onError(error) {
      const eventThreadId = activeRunThreadIdRef.current;
  ...
  const sendMessage = useCallback(
    async (
      ...
      const optimisticAdditionalKwargs = {
        ...
        // Add timestamp to optimistic messages for correct sorting
      ...
      if (optimisticFiles.length > 0 && !hideFromUI) {
        ...
        newOptimistic.push({
          ...
          additional_kwargs: { element: "task", timestamp: nowTimestamp },
        });
      }
