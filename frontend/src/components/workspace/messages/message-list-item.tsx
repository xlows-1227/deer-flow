...
function MessageContent_({
  ...
  const contentToDisplay = useMemo(() => {
    if (isHuman) {
      const result = rawContent ? stripUploadedFilesTag(rawContent) : "";
      // Debug: log human message content
      if (typeof window !== 'undefined' && (window as any).__DEBUG_MESSAGE_GROUPS__) {
        console.log('[DEBUG] Human message content:', {
          rawContent: typeof rawContent === 'string' ? rawContent.substring(0, 100) : JSON.stringify(rawContent).substring(0, 100),
          contentToDisplay: result.substring(0, 100),
  ...
  const toolOmissionResult = useMemo(() => {
    ...
    // real tool cards (including "使用 xxx 工具") — don't also emit
    // the omission banner to avoid showing the same tool call twice.
