...
Communicate with a running Friday instance via its HTTP API. Friday is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

Friday exposes two API surfaces behind an Nginx reverse proxy:

| Service        | Direct Port | Via Proxy                        | Purpose                          |
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$FRIDAY_GATEWAY_URL`          | REST endpoints and embedded agent runtime |
| LangGraph-compatible API | 8001 | `$FRIDAY_LANGGRAPH_URL`       | Agent threads, runs, streaming   |
...
| `FRIDAY_GATEWAY_URL`  | `${FRIDAY_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `FRIDAY_LANGGRAPH_URL`| `${FRIDAY_URL}/api/langgraph`          | LangGraph API base (threads, runs) |
...
FRIDAY_GATEWAY_URL="${FRIDAY_GATEWAY_URL:-$FRIDAY_URL}"
FRIDAY_LANGGRAPH_URL="${FRIDAY_LANGGRAPH_URL:-$FRIDAY_URL/api/langgraph}"
```
...
Verify Friday is running:

```bash
curl -s "$FRIDAY_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$FRIDAY_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$FRIDAY_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$FRIDAY_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$FRIDAY_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$FRIDAY_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$FRIDAY_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$FRIDAY_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$FRIDAY_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$FRIDAY_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$FRIDAY_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$FRIDAY_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
...
```bash
bash /path/to/skills/claude-to-friday/scripts/chat.sh "Your question here"
```
...
- If health check fails, Friday is not running. Inform the user they need to start it.
