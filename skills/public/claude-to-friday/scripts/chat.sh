#!/usr/bin/env bash
# chat.sh — Send a message to Friday and collect the streaming response.
#
# Usage:
...
#   FRIDAY_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: Friday is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure Friday is running. Start it with: cd <friday-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from Friday: {data_str}", file=sys.stderr)
            sys.exit(1)
